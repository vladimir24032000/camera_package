...
class Camera
  ...
  private void startCapture(boolean record, boolean stream, @NonNull final Result result) throws CameraAccessException {
  ...
  public void prepareRecording(@NonNull Result result) {
    final File outputDir = applicationContext.getCacheDir();
    try {
    ...
    try {
      startCapture(true, false, result);
     } catch (CameraAccessException e) {
    recordingVideo = false;
    captureFile = null;
    result.error("videoRecordingFailed", e.getMessage(), null);
